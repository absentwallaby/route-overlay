...
- `metersPoints` (route coordinates in local equirectangular meters) use y-down (south-positive), matching the map photo's pixel y convention. This is required for the similarity transform to fit correctly — mixing a y-up and y-down coordinate system produces a mirrored/flipped overlay since a pure rotation+scale transform can't represent a reflection. The mini route diagram's `toScreen` renders these directly (no extra flip) since north already ends up at the top.
